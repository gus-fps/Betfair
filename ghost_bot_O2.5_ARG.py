...
MIN_ODDS        = _config["o25arg_min_odds"]
MAX_ODDS        = _config["o25arg_max_odds"]
ALLOWED_LEAGUES = _config["o25arg_allowed_leagues"]
EXCLUDED_TEAMS  = _config["o25arg_excluded_teams"]
EXCLUDED_TEAMS_SET = set(t.lower() for t in EXCLUDED_TEAMS)

# ==========================================
# 3. LEDGER INITIALIZATION
# ==========================================
CSV_FILE = os.path.join(_script_dir, "paper_trading_ledger_o25_arg.csv")
...
try:
    ...
    raise SystemExit(1)
print("✅ Logged in successfully. Firing up the O2.5 ARG strategy engine...\n")
                ...
                if runner.ex.available_to_back:
                    ...
                    if MIN_ODDS <= best_back_price <= MAX_ODDS:
                        ...
                        if unique_id not in placed_market_selection_ids:
                            ...
                            print(f"🟢 [PAPER BET] {match_name} | Back Over 2.5 Goals @ {best_back_price} | Stake: £{PAPER_STAKE}")
